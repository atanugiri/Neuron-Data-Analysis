% Date: 07/15/2023
% Author: Atanu Giri
% This script finds all neurons from same session
...
neuronType = input("Enter the neuron type for analysis: 'FSI', 'Striosome', " + ...
    "'PLS', 'PL', or 'Matrix': ",'s');
if strcmpi(neuronType, 'FSI')
    neuronIdx = twdb_lookup(database, 'index', ...
        'key', 'tetrodeType', 'dms', ...
        'key', 'neuron_type', 'SWN', ...
...
elseif strcmpi(neuronType, 'Striosome')
    neuronIdx = twdb_lookup(database, 'index', 'key', 'tetrodeType', 'dms', ...
...
elseif strcmpi(neuronType, 'PLS')
    neuronIdx = twdb_lookup(database, 'index', 'key', 'tetrodeType', 'pl', 'grade', 'final_michael_grade', 1, 5, ...
...
% Add tasktype filter
specificTask = input('Do you want to analyze a specific task? (y/n) ', 's');
if strcmpi(specificTask, 'y')
    task = input("Enter the tasktype you want to analyze ('CB', 'EQR', 'Rev CB', 'TR'): ", 's');
    % twdb_lookup function with taskType filter
    neuronIdxByTask = twdb_lookup(database, 'index', 'key', 'taskType', sprintf('%s',task));
...
concRange = input('Do you want to analyze a concentration range? (y/n) ', 's');
if strcmpi(concRange, 'y')
    ...
    neuronIdxByConc = twdb_lookup(database, 'index', 'grade', 'conc', lowConc, highConc);
...
for neuron = 1:numel(neuronIdx)
    neuronData = database(neuronIdx(neuron));
...
parts = strsplit(dataTable, 'twdb_');
